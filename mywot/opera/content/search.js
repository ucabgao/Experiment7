...
wot.search = {
            ...
            if (match.attribute && match.attribute.length) {
                for (var i = 0; i < match.attribute.length; ++i) {
                    ...
                    if (!elem.hasAttribute(match.attribute[i].name) ||
                        !this.matchregexp(match.attribute[i], elem.getAttribute(match.attribute[i].name))) {
                        return false;
                    }
                }
            }
        ...
        try {
            ...
            if (match.element == "$frame") {
                ...
                if (/^#/.test(match.element)) {
                    set.push(docelem.getElementById(match.element.replace(/^#/, "")));
                }
                else {
    ...
    addstyle: function (css, frame, id) {
        try {
            ...
            style.innerText = css;
            var insertpoint = frame.document.getElementsByTagName("head") ||
            ...
            wot.components.forEach(function (item) {
                ...
                switch (wot.search.settings.search_type) {
                    case wot.searchtypes.optimized:
                        var type = wot.getwarningtypeforcomponent(item.name, data, wot.search.settings);
    ...
    getcss: function (rule, obj) {
        ...
        if (rule.style && !this.added[obj.target]) {
            ...
            css = this.formatcss(rule.style)
                ...
                .replace(/IMAGE/g, wot.files[wot.geticon(r, 16, this.settings.accessible, true)]);
        }
        return css;
    },
        ...
        if (this.matchrule(data.rule, frame)) {
            this.processframe(data.rule, frame, function (targets) {
                ...
                if (data.rule.prestyle) {
                    wot.search.addstyle(wot.search.formatcss(data.rule.prestyle), frame, wot.search.getname("prestyle"));
                }
                if (data.rule.popup && data.rule.popup.match &&
                    data.rule.popup.match.length) {
                    var elem = wot.search.findmatchingelement(rule.popup.match[0], frame);
                    if (elem) {
                ...
                wot.post("search", "get", { id: id, rule: data.rule, targets: targets });
            });
        }
        if (data.rule.dynamic || frame.frameElement) {
            var handler = {
                handleEvent: function () {
                    ...
                    frame.document.removeEventListener("DOMNodeInserted", this, false);
    ...
    onclickrating: function (event) {
        try {
            var target = event.target.getAttribute(wot.search.getattrname("target"));
            if (target) {
